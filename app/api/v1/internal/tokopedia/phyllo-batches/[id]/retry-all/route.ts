...
import { retryAllPhylloBatchItems } from "@/lib/tokopedia-phyllo-batch"
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  after(() =>
    retryAllPhylloBatchItems(id).catch((err) =>
      console.error("[tokopedia/phyllo-batches/retry-all] failed:", err),
    ),
  )
