...
/** Phyllo-only view of the shared scraper test runs. */
const PROVIDER = "phyllo"
...
export async function GET(req: NextRequest) {
  ...
  const where = eq(tiktokScraperTestRun.provider, PROVIDER)
  ...
  const [rows, [{ total }]] = await Promise.all([
    db
      ...
      .limit(limit)
      .offset(offset),
    db.select({ total: count() }).from(tiktokScraperTestRun).where(where),
  ])

  return NextResponse.json({ runs: rows, total, limit, offset })
}

// The provider is fixed here — this section only tests Phyllo.
const inputSchema = scraperTestInputSchema.omit({ provider: true })
...
export async function POST(req: NextRequest) {
  ...
  // Awaited, not deferred — the tester needs to see immediately whether Phyllo
  // accepted the trigger. The callback lands separately.
  const run = await createScraperTestRun({ ...parsed.data, provider: PROVIDER })
