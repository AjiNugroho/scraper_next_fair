...
export async function GET(req: NextRequest) {
  ...
  const [rows, [{ total }]] = await Promise.all([
    db
      ...
      .orderBy(desc(phylloScrapeRequest.createdAt))
      .limit(limit)
      .offset(offset),
    db.select({ total: count() }).from(phylloScrapeRequest),
  ])
...
export async function POST(req: NextRequest) {
  ...
  await db.insert(phylloScrapeRequest).values(
    data.map((item) => ({
      listenGroupId,
      requestDataId,
  ...
  // Phyllo requests feed the same mobile-worker hashtag pool — the workers are what
  // collect the video URLs a phyllo job later scrapes.
  await db
...
export async function DELETE(req: NextRequest) {
  ...
  await db.delete(phylloScrapeRequest).where(inArray(phylloScrapeRequest.id, parsed.data.ids))
