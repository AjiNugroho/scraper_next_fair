...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const updates: Partial<typeof phylloScrapeRequest.$inferInsert> = {}
  ...
  const rows = await db
    .update(phylloScrapeRequest)
    .set(updates)
    .where(eq(phylloScrapeRequest.id, id))
    .returning()
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = await db
    .delete(phylloScrapeRequest)
    .where(eq(phylloScrapeRequest.id, id))
    .returning({ id: phylloScrapeRequest.id })
