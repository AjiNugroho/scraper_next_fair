...
function DeleteBulkDialog({
  ...
  const deleteMany = useDeleteTokopediaRequests()
...
export function TokopediaRequestsManagement() {
  const [page, setPage] = useState(0)
  const [extrasDialog, setExtrasDialog] = useState<{
    open: boolean
    extras: Record<string, unknown> | null
  }>({ open: false, extras: null })
  const [editRequest, setEditRequest] = useState<TokopediaHashtagRequest | null>(null)
  const [deleteRequest, setDeleteRequest] = useState<TokopediaHashtagRequest | null>(null)
  const [rowSelection, setRowSelection] = useState<RowSelectionState>({})
  const [bulkDeleteOpen, setBulkDeleteOpen] = useState(false)

  const { data, isLoading, isError } = useTokopediaRequests({
    limit: PAGE_SIZE,
  ...
  const columns = useMemo<ColumnDef<TokopediaHashtagRequest>[]>(
    () => [
      {
        id: "select",
        header: ({ table }) => (
