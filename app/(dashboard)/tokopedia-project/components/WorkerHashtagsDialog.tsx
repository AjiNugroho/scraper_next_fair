...
export function WorkerHashtagsDialog({ worker, open, onOpenChange }: WorkerHashtagsDialogProps) {
  const { data, isLoading } = useQuery({
    queryKey: ["tokopedia-worker-hashtags", worker?.id],
    enabled: open && !!worker,
    queryFn: async () => {
      const res = await fetch(
        `/api/v1/internal/tokopedia/hashtags?worker_id=${encodeURIComponent(worker!.id)}&limit=200`,
      )
