...
import type { TokopediaVideoResult } from "../datahooks/useTokopediaResults"
import {
  useTokopediaResults,
  useDeleteTokopediaResult,
  useDeleteTokopediaResults,
} from "../datahooks/useTokopediaResults"
...
function DeleteSingleDialog({
  ...
  row: TokopediaVideoResult | null
  open: boolean
  onOpenChange: (v: boolean) => void
}) {
  const [confirm, setConfirm] = useState("")
  const deleteOne = useDeleteTokopediaResult()
...
function DeleteBulkDialog({
  ...
  const [confirm, setConfirm] = useState("")
  const deleteMany = useDeleteTokopediaResults()
...
export function ResultsTable({ hashtags }: { hashtags: string[] }) {
  ...
  const [deleteSingle, setDeleteSingle] = useState<TokopediaVideoResult | null>(null)
  const [bulkOpen, setBulkOpen] = useState(false)

  const resetPage = useCallback(() => setPage(0), [])

  const { data, isLoading, isError } = useTokopediaResults({
    search,
    hashtag,
    limit: PAGE_SIZE,
  ...
  const columns = useMemo<ColumnDef<TokopediaVideoResult>[]>(
    () => [
      {
        id: "select",
        header: ({ table }) => (
