...
import type { SubmitPhylloRequestInput } from "../datahooks/usePhylloRequests"
...
type CsvDataItem = SubmitPhylloRequestInput["data"][number]
...
function parseRows(rows: string[][]): { data: ParsedRow[]; errors: string[] } {
  ...
  const hashtagIdx =
    header.indexOf("hashtag") !== -1 ? header.indexOf("hashtag") : header.indexOf("identifier")
  if (hashtagIdx === -1) {
...
export function ImportCsvDialog() {
  ...
  async function handleImport() {
    ...
    for (const batch of batches) {
      await submitRequest.mutateAsync(batch)
  ...
  function handleDownloadTemplate() {
    ...
    link.href = url
    link.download = "phyllo-scrape-requests-template.csv"
    link.click()
    ...
    <Dialog open={open} onOpenChange={(v) => (v ? setOpen(true) : handleClose())}>
      ...
      <DialogContent className="sm:max-w-3xl max-h-[90vh] flex flex-col gap-0">
        <DialogHeader className="shrink-0 pb-4">
          <DialogTitle>Import Phyllo Requests from CSV</DialogTitle>
          <DialogDescription>
        ...
        <div className="flex flex-col gap-4 overflow-y-auto pr-1 pb-2">
          ...
          <Field>
            <FieldLabel htmlFor="phyllo-csv-file">CSV File</FieldLabel>
            <Input
              id="phyllo-csv-file"
              ref={fileInputRef}
              type="file"
