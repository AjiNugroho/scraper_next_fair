...
import { usePhylloRequests, useDeletePhylloRequests } from "../datahooks/usePhylloRequests"
...
function DeleteBulkDialog({
  ...
  const deleteMany = useDeletePhylloRequests()
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>
            Delete {ids.length} request{ids.length !== 1 ? "s" : ""}
          </DialogTitle>
          <DialogDescription>
            This action cannot be undone, and it also removes any Phyllo job history for these
            requests. Type{" "}
...
export function PhylloRequestsManagement() {
  const [page, setPage] = useState(0)
  const [extrasDialog, setExtrasDialog] = useState<{
    open: boolean
    extras: Record<string, unknown> | null
  }>({ open: false, extras: null })
  const [editRequest, setEditRequest] = useState<PhylloScrapeRequest | null>(null)
  const [deleteRequest, setDeleteRequest] = useState<PhylloScrapeRequest | null>(null)
  const [rowSelection, setRowSelection] = useState<RowSelectionState>({})
  const [bulkDeleteOpen, setBulkDeleteOpen] = useState(false)

  const { data, isLoading, isError } = usePhylloRequests({
    limit: PAGE_SIZE,
  ...
  const columns = useMemo<ColumnDef<PhylloScrapeRequest>[]>(
    () => [
      {
        id: "select",
        header: ({ table }) => (
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-1 justify-end">
            <Button
              ...
              size="icon"
              className="h-7 w-7"
              onClick={() => setEditRequest(row.original)}
            >
            ...
            <Button
              ...
              onClick={() => setDeleteRequest(row.original)}
            >
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                ...
                >
                  No Phyllo requests submitted yet.
                </TableCell>
              </TableRow>
            ) : (
