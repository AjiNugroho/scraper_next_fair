...
import {
  usePhylloEligibleHashtags,
  usePreviewPhylloScrapeJob,
  useTriggerPhylloScrapeJob,
  type PhylloScrapeJobTriggerFilters,
} from "../datahooks/usePhylloScrapeJobs"

function formatDate(iso: string) {
  return new Date(iso).toLocaleString()
}

export function TriggerPhylloScrapeJobDialog() {
  ...
  const [preview, setPreview] = useState<{
    filters: PhylloScrapeJobTriggerFilters
  ...
  const { data: hashtagsData, isLoading: hashtagsLoading } = usePhylloEligibleHashtags()
  ...
  const previewJob = usePreviewPhylloScrapeJob()
  const triggerJob = useTriggerPhylloScrapeJob()
  ...
  function buildFilters(): PhylloScrapeJobTriggerFilters {
    const hashtags =
        ...
        {preview ? (
          <>
            <DialogHeader className="shrink-0 pb-4">
              ...
              <DialogDescription>
                Review what will be sent to Phyllo Scraper before running.
              </DialogDescription>
            </DialogHeader>
          ...
          <>
            <DialogHeader className="shrink-0 pb-4">
              ...
              <DialogDescription>
                Choose which hashtags and date range to send to Phyllo Scraper.
              </DialogDescription>
            </DialogHeader>
