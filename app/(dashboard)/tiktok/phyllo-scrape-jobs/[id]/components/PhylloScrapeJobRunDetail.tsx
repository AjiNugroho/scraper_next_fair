...
import {
  usePhylloScrapeJobRun,
  useRetryPhylloScrapeJobRun,
  type PhylloScrapeJobRun,
  type PhylloScrapeJobRunItem,
...
export function PhylloScrapeJobRunDetail({ id }: { id: string }) {
  const [page, setPage] = useState(0)
  const [statusFilter, setStatusFilter] = useState("all")

  const { data, isLoading, isError } = usePhylloScrapeJobRun(id, {
    status: statusFilter === "all" ? undefined : statusFilter,
    limit: PAGE_SIZE,
    offset: page * PAGE_SIZE,
  })
  const retry = useRetryPhylloScrapeJobRun()

  const run: PhylloScrapeJobRun | undefined = data?.run
  const items = data?.items ?? []
  const total = data?.total ?? 0
  const totalPages = Math.ceil(total / PAGE_SIZE)

  const columns = useMemo<ColumnDef<PhylloScrapeJobRunItem>[]>(
    () => [
      {
        accessorKey: "hashtag",
        header: "Hashtag",
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span
            className="text-sm max-w-[280px] truncate block"
      ...
      },
      {
        ...
        header: "Status",
        cell: ({ row }) => (
          <StatusBadge status={row.original.status} styles={ITEM_STATUS_STYLES} />
        ),
      },
      {
