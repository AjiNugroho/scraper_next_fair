...
import { useBeautyDispatchRuns, type BeautyDispatchRun } from "../datahooks/useBeautyDispatcher"
import { TriggerBeautyDispatchDialog } from "./TriggerBeautyDispatchDialog"
...
function HashtagsCell({ hashtags }: { hashtags: string[] | null }) {
  if (!hashtags || hashtags.length === 0) return <span className="text-muted-foreground">—</span>
...
const columns: ColumnDef<BeautyDispatchRun>[] = [
  {
  ...
  },
  {
    ...
    header: "Requests",
    cell: ({ row }) =>
      row.original.filterRequestIds === null ? (
        <div className="flex items-center gap-1.5">
          <Badge variant="outline">All</Badge>
          <span className="text-sm text-muted-foreground tabular-nums">
            {row.original.requestsCount}
          </span>
        </div>
      ) : (
        <span className="text-sm tabular-nums">{row.original.requestsCount} selected</span>
      ),
  },
  {
    id: "hashtags",
    header: "Hashtags",
    cell: ({ row }) => <HashtagsCell hashtags={row.original.hashtags} />,
  },
  {
    id: "dateRange",
    header: "Date Range",
...
export function BeautyDispatchRunsTable() {
  const router = useRouter()
  const [page, setPage] = useState(0)
  const { data, isLoading, refetch, isFetching } = useBeautyDispatchRuns(page, PAGE_SIZE)
            ...
            {isLoading ? (
              ...
              table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  onClick={() => router.push(`/tiktok-beauty-scraper/video-dispatcher/${row.original.id}`)}
                >
