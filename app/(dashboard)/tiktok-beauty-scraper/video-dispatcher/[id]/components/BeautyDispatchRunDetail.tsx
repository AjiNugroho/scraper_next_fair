...
import {
  useBeautyDispatchRun,
  useRetryBeautyDispatchRun,
  type BeautyDispatchRunItem,
...
export function BeautyDispatchRunDetail({ id }: { id: string }) {
  const [page, setPage] = useState(0)
  const [statusFilter, setStatusFilter] = useState("all")

  const { data, isLoading, isError } = useBeautyDispatchRun(id, {
    status: statusFilter === "all" ? undefined : statusFilter,
    limit: PAGE_SIZE,
    offset: page * PAGE_SIZE,
  })
  const retry = useRetryBeautyDispatchRun()
  ...
  const columns = useMemo<ColumnDef<BeautyDispatchRunItem>[]>(
    () => [
      {
        accessorKey: "hashtag",
        header: "Hashtag",
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-1.5 whitespace-nowrap">
            <Badge variant="outline" className="font-normal tabular-nums">
              LG {row.original.listenGroupId ?? "—"}
            </Badge>
            <Badge variant="outline" className="font-normal tabular-nums">
              RD {row.original.requestDataId ?? "—"}
            </Badge>
          </div>
        ),
      },
      {
        ...
        header: "URL",
        cell: ({ row }) => (
          <span className="text-sm max-w-[280px] truncate block" title={row.original.url}>
            {row.original.url}
          </span>
        ),
      },
  ...
  return (
    <Card className="bg-background border-none shadow-none ring-0 space-y-4">
      <div className="flex flex-col gap-3">
        <Link
          href="/tiktok-beauty-scraper/video-dispatcher"
          className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground w-fit"
        >
          <ArrowLeft className="h-3.5 w-3.5" />
          Back to Dispatch Runs
        </Link>
            ...
            {run && (
              <div className="flex flex-wrap items-center gap-3 mt-2 text-sm">
                <span className="text-muted-foreground">
                  Duration:{" "}
                  <strong className="text-foreground">{duration(run.startedAt, run.completedAt)}</strong>
                </span>
                <span className="text-muted-foreground">
                  Requests:{" "}
                  <strong className="text-foreground">
                    {run.filterRequestIds === null ? `All (${run.requestsCount})` : run.requestsCount}
                  </strong>
                </span>
                <span className="text-muted-foreground">
                  URLs Sent:{" "}
